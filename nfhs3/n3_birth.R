...
birth_vars <- readxl::read_excel("data/mapping_ext.xlsx",sheet="ncm_variables") %>% 
  rename("selected" = iabr52dt) %>% 
  dplyr::select(new_var,selected) %>% 
  dplyr::filter(!is.na(selected))

n3_birth <- read_dta(paste0(path_dhs_folder,"/IA/IABR52DT/IABR52FL.dta"),col_select = birth_vars$selected) %>% 
...
saveRDS(n3_birth,paste0(path_nfhs_malnutrition_paper,"/working/n3_birth.RDS"))
