...
population_vars <- readxl::read_excel("data/mapping_ext.xlsx",sheet="ncm_variables") %>% 
  rename("selected" = iapr52dt) %>% 
  dplyr::select(new_var,selected) %>% 
  dplyr::filter(!is.na(selected))

n3_population <- read_dta(paste0(path_dhs_folder,"/IA/IAPR52DT/IAPR52FL.dta"),col_select = population_vars$selected) %>% 
...
saveRDS(n3_population,paste0(path_nfhs_malnutrition_paper,"/working/n3_population.RDS"))
