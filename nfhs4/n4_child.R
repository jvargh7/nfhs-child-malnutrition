...
child_vars <- readxl::read_excel("data/mapping_ext.xlsx",sheet="ncm_variables") %>% 
  rename("selected" = iapr74dt_child) %>% 
  dplyr::select(new_var,selected) %>% 
  dplyr::filter(!is.na(selected))

n4_child <- read_dta(paste0(path_dhs_folder,"/IA/IAPR74DT/IAPR74FL.dta"),col_select = child_vars$selected) %>% 
...
saveRDS(n4_child,paste0(path_nfhs_malnutrition_paper,"/working/n4_child.RDS"))
