...
individual_vars <- readxl::read_excel("data/mapping_ext.xlsx",sheet="ncm_variables") %>% 
  rename("selected" = iair74dt) %>% 
  dplyr::select(new_var,selected) %>% 
  dplyr::filter(!is.na(selected))

n4_individual <- read_dta(paste0(path_dhs_folder,"/IA/IAIR74DT/IAIR74FL.dta"),col_select = individual_vars$selected) %>% 
...
saveRDS(n4_individual,paste0(path_nfhs_malnutrition_paper,"/working/n4_individual.RDS"))
